...
highest_sales = df.T['Total Sale'].idxmax()
lowest_sales = df.T['Total Sale'].idxmin()

print(f"Produce {highest_sales} has highest sales with sales $:", int(df.T['Total Sale'][highest_sales]))
print(f"Produce {lowest_sales} has lowest sales with sales $:", int(df.T['Total Sale'][lowest_sales]))
...
print(df.loc[['Quantity Sold','Total Sale'],['Orange','Beets']])
total_sales = int(df.T.loc[['Orange','Beets'],['Total Sale']].sum())
total_quantity = int(df.T.loc[['Orange','Beets'],["Quantity Sold"]].sum())
...
print(df.loc[['Total Sale'],'Apples':'Lettuce'])
...
df.at['Quantity Sold', 'Apricots'] = 11955
df.at['Total Sale', 'Apricots'] = 44353.05
print(df['Apricots'])
print(df)
...
new_df = newDF = (df.T[df.T['Quantity Sold'].between(11500,12000)]).T
print(new_df)
